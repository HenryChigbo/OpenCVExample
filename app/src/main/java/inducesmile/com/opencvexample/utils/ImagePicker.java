...
public class ImagePicker {
    ...
    public static Intent getPickImageIntent(Context context) {
        ...
        Uri contentUri = FileProvider.getUriForFile(context, "inducesmile.com.opencvexample.fileprovider", getTempFile(context));
        ...
        if (intentList.size() > 0) {
            chooserIntent = Intent.createChooser(intentList.remove(intentList.size() - 1), "Pick image intent text");
    ...
    public static Bitmap getImageFromResult(Context context, int resultCode, Intent imageReturnedIntent) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (isCamera) {
                selectedImage = FileProvider.getUriForFile(context, "inducesmile.com.opencvexample.fileprovider", imageFile);
                bitmap = BitmapFactory.decodeFile(imageFile.getAbsolutePath());
            } else {
                selectedImage = imageReturnedIntent.getData();
                try {
                    bitmap = uriToBitmap(context, selectedImage);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
